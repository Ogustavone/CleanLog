...
filtro = registro if funcionario_selecionado == "Todos" else registro[registro['Funcionário'] == funcionario_selecionado]
...
# --- EXIBIÇÃO ---
tipo_visao = st.radio("Tipo de Visão", ["Registro Diário", "Visão Geral"], horizontal=True)
cols_financeiras = ["Custo_Dia", "Total (R$)"]

if df_final.empty:
    st.warning(f'⚠️ Nenhum registro encontrado para o período: "{dias[0]} de {meses[0].capitalize()}" a "{dias[1]} de {meses[1].capitalize()}" de {ano}.')
else:
    ...
    if tipo_visao == "Registro Diário":
        ...
        st.dataframe(
            diario[cols].style.format({'Custo_Dia': 'R$ {:.2f}'}) if mostrar_valores else diario[cols], 
            use_container_width=True, 
            hide_index=True
        )
        
        csv = diario[cols].to_csv(index=False).encode('utf-8')
        st.download_button(f"📥 Baixar CSV (Diário)", csv, f"diario_{nome_periodo}.csv", "text/csv")
    
    else:
        geral = df_final.groupby('Funcionário').agg({
            'Data': 'nunique',
            'Horas_Reg_Num': 'sum',
            'Horas_Ext_Num': 'sum',
            'Custo_Dia': 'sum'
        ...
        geral.columns = ['Funcionário', 'Dias Trabalhados', 'H. Regulares', 'H. Extras', 'Total (R$)']
        ...
        st.download_button(f"📥 Baixar CSV (Geral)", csv, f"fechamento_{nome_periodo}.csv", "text/csv")