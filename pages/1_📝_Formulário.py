...
from src.utils import obter_dias_do_mes, preparar_registros_para_envio, formatar_horario
from src.database import adicionar_dataframe_na_planilha
from datetime import datetime
import calendar
from src.components import (
    configurar_pagina, 
    renderizar_seletor_calendario
    )
...
with st.container(border=True):
    ...
    colaborador = col_nome.text_input("Nome do Colaborador", placeholder="Ex: João Silva")
    lista_meses = [calendar.month_name[i].capitalize() for i in range(1, 13)]
    mes_selecionado = col_mes.selectbox("Mês", lista_meses, index=datetime.now().month - 1)
    ...
    if st.button("Enviar para Planilha", type="primary", use_container_width=True):
        if not colaborador:
            ...
            if df_envio.empty:
                st.warning("Nenhum turno marcado para o mês selecionado. Verifique os lançamentos.")
            else:
                try:
                    adicionar_dataframe_na_planilha(df_envio)
                    st.success(f"Dados de {colaborador} enviados para o Google Sheets com sucesso! ✅")
